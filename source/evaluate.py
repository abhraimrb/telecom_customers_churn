...
def evaluate(param_yaml_path, model, data, data_category, live):
    ...
    avg_prec = metrics.average_precision_score(y['Churn'].values, predictions_proba[:, 1])
    roc_auc = metrics.roc_auc_score(y, predictions_proba[:, 1])
    acc = metrics.accuracy_score(y, y_pred)
    f1 = metrics.f1_score(y, y_pred)
    prec_score = metrics.precision_score(y, y_pred)
    rec_score = metrics.recall_score(y, y_pred)

    if not live.summary:
        live.summary = {"avg_prec": {}, "roc_auc": {},"acc_score": {}, "f1": {},"prec_score": {}, "rec_score": {}}
    live.summary["avg_prec"][data_category] = avg_prec
    live.summary["roc_auc"][data_category] = roc_auc
    live.summary["acc_score"][data_category] = acc
    live.summary["f1"][data_category] = f1
    live.summary["prec_score"][data_category] = prec_score
    live.summary["rec_score"][data_category] = rec_score


    # ... and plots...
    live.log_sklearn_plot("roc", y, predictions_proba[:, 1], name=f"roc/{data_category}")

    precision, recall, prc_thresholds = \
            metrics.precision_recall_curve(y, predictions_proba[:, 1])
...
if __name__=="__main__":
    ...
    axes.set_yticks(range(len(indices)), [features[i] for i in indices])
